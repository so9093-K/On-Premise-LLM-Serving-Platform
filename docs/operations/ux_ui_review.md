...
### 모니터링 UX (0.1.0-rc.1)
- 초기 Grafana 3개 대시보드에 한국어 설명, `timezone: browser`, 대시보드 간 네비게이션 링크를 추가했고, 이후 운영 기준은 6개 dashboard와 variable-backed Git-managed JSON으로 확장했다. Grafana home은 `Serving Cockpit`이며 GPU dashboard는 GPU/OOM drill-down으로 유지한다.
