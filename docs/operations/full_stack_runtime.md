...
`Request Log Explorer`는 `request_id_filter`/`client_host_filter`/`route_filter`/`error_code_filter`(정규식, 기본값 `.*`) 대시보드 변수로 특정 요청 하나·IP·route·error code만 좁혀보는 상세 조회를 지원한다. 에러 응답은 `X-Error-Code`/`X-Request-Id` 응답 헤더를 붙이고(`errors.py`의 `error_response_headers`), 접근 로그가 그 값을 그대로 옮겨 적는다(`logging_policy.py`) — 그래서 (1) status_code 하나(예: 503)에 여러 code(MODEL_UNAVAILABLE/QUEUE_TIMEOUT/CIRCUIT_OPEN/MAIN_MODEL_SWITCH_IN_PROGRESS 등)가 몰려도 로그에서 바로 구분되고, (2) 클라이언트가 `x-request-id`를 안 보낸 요청도 에러 시 새로 발급된 request_id가 응답 바디·헤더·로그에서 항상 일치해, 사용자가 보고한 request_id 그대로 로그를 찾을 수 있다. "Errors" 패널은 `| json` 파싱 기반이라 gateway/risk-adapter의 구조화 로그만 잡으므로, vLLM처럼 JSON이 아닌 uvicorn access log를 쓰는 컨테이너의 4xx/5xx는 별도의 "Non-JSON Container Errors" 패널(텍스트 패턴 매칭, best-effort)에서 확인한다.
