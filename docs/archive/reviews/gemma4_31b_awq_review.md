...
이 문서는 모델 교체 의사결정의 과거 배경만 보존한다. 현재 Gateway의 `/v1/chat/completions`는 `local-main`이라는 served model name을 유지하되, upstream checkpoint는 RedHatAI FP8 Dynamic Gemma 4 26B-A4B로 교체되었다.
...
- replacement: `RedHatAI/gemma-4-26B-A4B-it-FP8-Dynamic`
