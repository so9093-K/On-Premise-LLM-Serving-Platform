...
`risk-siren` 모델 카드는 lineage와 retired 호환성 추적을 위해 남겨두지만, 기본 Gateway `/v1/models` 응답과 aggregate execution에는 포함하지 않는다. `risk.aggregate_signal`은 개별 모델 capability가 아니라 Risk Adapter aggregate route의 동작으로 문서화한다.
