...
추가를 승인하려면 테스트 설명이나 PR에 **막는 손실**(예: 공개 API 호환성
파손, 인증 우회, 민감정보 노출, 상태 전환 실패)과 그 위험을 소유하는 계층
(`validator`, unit test, live runtime check)을 한 줄로 남긴다. "이번 변경을
확인한다", "값이 현재와 같다", "나중에 필요할 수 있다"는 근거가 아니다.
기존 방어선이 있으면 새 테스트를 더하지 않고 그 방어선을 고친다.
