...
- [vLLM PR #15316](https://github.com/vllm-project/vllm/pull/15316) (2025-03-22): V1 engine fix — `xgrammar:disable-any-whitespace` backend 지원
- 두 PR 모두 현재 pinned image(`vllm/vllm-openai:gemma4-0505-cu129`, 2025-05-05 빌드)에 포함됨
...
vLLM main LLM structured output backend를 `auto`(xgrammar)에서 `xgrammar:disable-any-whitespace`로 변경한다.

이 값은 xgrammar를 유지하면서 `any_whitespace` 기능만 비활성화하는 vLLM 팀의 targeted fix다. outlines로 전환하지 않는다.

변경 위치:

| 파일 | 변경 |
|---|---|
| `configs/model_serving.yaml` | `structured_outputs.backend: "xgrammar:disable-any-whitespace"` |
| `ops/compose/full-stack.private-network.yaml` | `--structured-outputs-config '{"backend":"xgrammar:disable-any-whitespace","enable_in_reasoning":true}'` |

두 파일은 source of truth가 분리되어 있으므로(model_serving.yaml은 application config, compose는 runtime config), 불일치를 CI에서 조기 감지하는 contract test를 추가한다: `test_compose_structured_outputs_config_matches_model_serving_yaml`.

## Consequences

| Positive | Negative |
|---|---|
| xgrammar `any_whitespace` stuck state 버그가 제거됨 | `xgrammar:disable-any-whitespace` 문자열은 vLLM 내부 파싱 컨벤션에 의존하므로 vLLM 버전 변경 시 재확인 필요 |
...
| compose ↔ model_serving.yaml 불일치를 CI가 즉시 탐지함 | |
