...
MIME type allowlist 검사(`image/jpeg`, `image/png`, `image/webp` 중 하나여야 함)는 유지된다. 변경 대상은 "MIME type으로 파서를 선택하는 것"이며, "MIME type을 검증하는 것"은 아니다.
...
- 이미지 처리 비용은 픽셀 수에 비례하므로, 6,422,528 픽셀(≈2688×2394) 이미지는 소형 이미지 대비 추론 시간이 증가한다. `max_model_len: 16384`에서 최대 이미지 토큰은 ≈2,048이며 나머지 ≈14,336이 텍스트 예산으로 남는다.
