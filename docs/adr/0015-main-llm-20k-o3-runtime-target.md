...
- 당시 필수 static/config 검증: `make validate`, `modelctl validate`, `validate_vllm_compose.py`, unit/contract tests, `render_vllm_commands.py --service main_llm`. 이 명령 표면은 이 ADR과 함께 superseded됐으며 현재 gate는 `make validate`·`make test`와 Compose preflight다.
