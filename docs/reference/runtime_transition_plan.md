...
Plan과 Apply는 서로 다른 admission 구현을 갖지 않는다. Runtime Controller가 기존 `gpu_budget.plan_activation`을 사용해 plan을 만들고, `plan_digest`를 PATCH body에 반드시 포함해야 한다. 실제 mutation 직전 GPU budget lock 안에서 같은 plan을 다시 계산하며 digest가 달라지면 실행하지 않고 `409 CONFLICT`와 `reason=RUNTIME_PLAN_CHANGED`를 반환한다.

Apply는 Plan에서 받은 `plan_digest` 없이는 실행되지 않는다. first-party Admin Console과 외부 자동화 모두 Plan을 먼저 요청하고 반환된 digest를 Apply에 전달한다. 이 digest는 클라이언트가 public projection을 다시 해시하기 위한 값이 아니라, sidecar가 검토 시점의 canonical transition snapshot을 apply-time 재계산과 비교하기 위한 opaque token이다.
