...
`make validate`와 `make test`는 Python 정책을 먼저 검사한다. 개발 환경 준비는 `make setup-dev`를 사용한다. `make doctor-dev`는 Compose·배포 shell helper까지 실행할 환경의 Bash 4 이상 여부를 별도로 확인한다. GitHub Actions는 `.python-version`에 기록된 Linux 기준 patch에서 major.minor를 계산해 macOS와 Ubuntu의 app/contract 검증에 사용한다. OS별 patch는 runner 제공 범위에 따르며 GPU 통합 검증은 별도 Linux/NVIDIA 환경에서 수행한다.
