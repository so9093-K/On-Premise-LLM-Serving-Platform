...
from ..services.runtime_controller_client import RuntimeControllerRequestError, RuntimeControllerUnavailableError
...
def runtime_controller_unavailable_response(exc: RuntimeControllerUnavailableError) -> JSONResponse:
    ...
    transport 예외 원문과 Runtime Controller 응답 본문이 들어가는데, 이 helper는 관리 API만이
...
def runtime_controller_request_error_response(exc: RuntimeControllerRequestError) -> JSONResponse:
    ...
    if exc.status_code == 503:
        # 연결 자체의 실패뿐 아니라 Runtime Controller가 준비되지 않아 돌려준 503도 같은
