...
def operator_status_bundle_markdown(document: dict[str, Any]) -> str:
    ...
    lines.extend([
        ...
        "",
        "| 서비스 키 | 모델 | Compose 서비스 | GPU utilization | max_model_len | max_num_seqs |",
        "|---|---|---|---:|---:|---:|",
    ])
    for row in gpu.get("per_runtime", []):
        lines.append(
            "| {service_key} | {logical_id} | {compose_service_name} | {gpu_memory_utilization} | {max_model_len} | {max_num_seqs} |".format(**row)
        )
