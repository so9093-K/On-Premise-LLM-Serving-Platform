...
def runtime_validation_matrix_checks(registry: "ModelRegistry") -> tuple["RuntimeValidationMatrixCheck", ...]:
    ...
    return (
        ...
        RuntimeValidationMatrixCheck(
            ...
            owner="gateway",
            validation="reasoning=true 요청이 reasoning을 종료한 뒤 json_schema-valid final answer를 반환한다.",
            artifact_file="reports/runtime/json-schema-with-reasoning-canary.md",
            runtime_validation_required=True,
            operator_action="Inspect the active Gemma4 template, reasoning parser, and max_tokens budget before changing the profile policy.",
            models=chat_models,
