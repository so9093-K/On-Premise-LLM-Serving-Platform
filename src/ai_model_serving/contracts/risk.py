...
from .common import ensure_request_object, ensure_response_object, normalize_complete_token_usage
...
def _validate_risk_category(category: Any, *, index: int) -> bool:
    if not isinstance(category, dict):
        raise ServiceError("UPSTREAM_RESPONSE_INVALID", f"risk response categories[{index}] must be an object.")
    required = {"code", "family", "detected", "confidence"}
    if not required.issubset(category):
        raise ServiceError("UPSTREAM_RESPONSE_INVALID", f"risk response categories[{index}] is missing required fields.")
    if not isinstance(category.get("detected"), bool):
        raise ServiceError("UPSTREAM_RESPONSE_INVALID", f"risk response categories[{index}].detected must be boolean.")
    ...
    if code is None:
        ...
        if category["detected"] is not False:
            raise ServiceError("UPSTREAM_RESPONSE_INVALID", f"risk response categories[{index}] safe category is inconsistent.")
        ...
        if family == "prompt_attack" and label != "<SAFE>":
            raise ServiceError("UPSTREAM_RESPONSE_INVALID", f"risk response categories[{index}] prompt safe label must be <SAFE>.")
        if family not in {"prompt_attack", "policy_risk", "data_exposure"}:
            raise ServiceError("UPSTREAM_RESPONSE_INVALID", f"risk response categories[{index}] family is not supported.")
    elif code in {"A1", "A2"}:
        if family != "prompt_attack" or label != f"<UNSAFE-{code}>":
            raise ServiceError("UPSTREAM_RESPONSE_INVALID", f"risk response categories[{index}] prompt risk code is inconsistent.")
    elif code in {"I1", "I2", "I3", "I4"}:
        if family != "policy_risk" or label != f"<UNSAFE-{code}>":
            raise ServiceError("UPSTREAM_RESPONSE_INVALID", f"risk response categories[{index}] policy risk code is inconsistent.")
    elif code in DATA_EXPOSURE_CODES:
        if family != "data_exposure":
            raise ServiceError("UPSTREAM_RESPONSE_INVALID", f"risk response categories[{index}] data exposure code must have family=data_exposure.")
        ...
        if category["detected"] and (not isinstance(label, str) or not label):
            raise ServiceError("UPSTREAM_RESPONSE_INVALID", f"risk response categories[{index}] detected data_exposure category must have a non-empty label.")
        ...
        if span_count is not None and (not isinstance(span_count, int) or span_count < 0):
            raise ServiceError("UPSTREAM_RESPONSE_INVALID", f"risk response categories[{index}].span_count must be a non-negative integer or null.")
    else:
        raise ServiceError("UPSTREAM_RESPONSE_INVALID", f"risk response categories[{index}].code is not supported.")
    ...
    if "source_model" in category:
        ...
        if sm is not None and (not isinstance(sm, str) or not sm):
            raise ServiceError("UPSTREAM_RESPONSE_INVALID", f"risk response categories[{index}].source_model must be a non-empty string or null.")
    if code in DATA_EXPOSURE_CODES:
        ...
        if "source_model" not in category or not isinstance(category.get("source_model"), str) or not category["source_model"]:
            raise ServiceError("UPSTREAM_RESPONSE_INVALID", f"risk response categories[{index}].source_model must be a non-empty string for data_exposure.")
        ...
        if not isinstance(category.get("source_model"), str) or not category["source_model"]:
            raise ServiceError("UPSTREAM_RESPONSE_INVALID", f"risk response categories[{index}].source_model must be a non-empty string.")
...
def _validate_risk_system_signal(signal: Any, *, index: int) -> bool:
    if not isinstance(signal, dict):
        raise ServiceError("UPSTREAM_RESPONSE_INVALID", f"risk response system_signals[{index}] must be an object.")
    required = {"code", "detected", "retryable"}
    if not required.issubset(signal):
        raise ServiceError("UPSTREAM_RESPONSE_INVALID", f"risk response system_signals[{index}] is missing required fields.")
    if signal.get("code") not in SYSTEM_RISK_CODES:
        raise ServiceError("UPSTREAM_RESPONSE_INVALID", f"risk response system_signals[{index}].code is not supported.")
    if not isinstance(signal.get("detected"), bool):
        raise ServiceError("UPSTREAM_RESPONSE_INVALID", f"risk response system_signals[{index}].detected must be boolean.")
    if not isinstance(signal.get("retryable"), bool):
        raise ServiceError("UPSTREAM_RESPONSE_INVALID", f"risk response system_signals[{index}].retryable must be boolean.")
...
def validate_risk_response(payload: Any) -> dict[str, Any]:
    ...
    if forbidden:
        ...
        raise ServiceError("UPSTREAM_RESPONSE_INVALID", f"risk response contains forbidden policy field(s): {names}.")
    missing = sorted(RISK_RESPONSE_REQUIRED_FIELDS.difference(payload))
    if missing:
        names = ", ".join(missing)
        raise ServiceError("UPSTREAM_RESPONSE_INVALID", f"risk response is missing required field(s): {names}.")

    if not isinstance(payload.get("assessment_id"), str) or not payload["assessment_id"].startswith("risk_"):
        raise ServiceError("UPSTREAM_RESPONSE_INVALID", "risk response assessment_id must start with risk_.")
    status = payload.get("status")
    if status not in RISK_RESPONSE_STATUS:
        raise ServiceError("UPSTREAM_RESPONSE_INVALID", "risk response status is not supported.")
    if not isinstance(payload.get("message"), str):
        raise ServiceError("UPSTREAM_RESPONSE_INVALID", "risk response message must be a string.")
    if "usage" in payload:
        usage = normalize_complete_token_usage(payload["usage"], reject_extra_fields=True)
        if usage is None:
            raise ServiceError(
                "UPSTREAM_RESPONSE_INVALID", "risk response usage must contain non-negative integer prompt_tokens, completion_tokens, and total_tokens only.",
            )
    ...
    if not isinstance(categories, list) or not isinstance(system_signals, list):
        raise ServiceError("UPSTREAM_RESPONSE_INVALID", "risk response categories and system_signals must be arrays.")
    ...
    for field in bool_fields:
        if not isinstance(payload.get(field), bool):
            raise ServiceError("UPSTREAM_RESPONSE_INVALID", f"risk response {field} must be boolean.")

    if payload["risk_detected"] != model_risk_detected or payload["model_risk_detected"] != model_risk_detected:
        raise ServiceError("UPSTREAM_RESPONSE_INVALID", "risk response model risk booleans are inconsistent.")
    if payload["system_signal_detected"] != system_signal_detected:
        raise ServiceError("UPSTREAM_RESPONSE_INVALID", "risk response system signal boolean is inconsistent.")
    if payload["attention_required"] != (model_risk_detected or system_signal_detected):
        raise ServiceError("UPSTREAM_RESPONSE_INVALID", "risk response attention_required is inconsistent.")
    if payload["assessment_complete"] != (status == "completed"):
        raise ServiceError("UPSTREAM_RESPONSE_INVALID", "risk response assessment_complete is inconsistent with status.")
    ...
    if strongest_code is not None and strongest_code not in MODEL_RISK_CODES | SYSTEM_RISK_CODES:
        raise ServiceError("UPSTREAM_RESPONSE_INVALID", "risk response strongest_code is not supported.")
    if strongest_code is not None and not (model_risk_detected or system_signal_detected):
        raise ServiceError("UPSTREAM_RESPONSE_INVALID", "risk response strongest_code requires a detected model or system signal.")
