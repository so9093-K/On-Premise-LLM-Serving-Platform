...
def _validate_response_format(value: Any, payload: dict[str, Any], policy: dict[str, Any] | None) -> None:
    ...
    if not isinstance(response_policy, dict) or response_policy.get("enabled") is not True:
        raise ServiceError("VALIDATION_ERROR", "response_format is not enabled for this model; omit response_format or switch to a profile that supports structured responses.")
    if not isinstance(value, dict):
        raise ServiceError("VALIDATION_ERROR", "response_format must be an object when provided.")
    ...
    if response_type not in allowed_types:
        ...
        raise ServiceError("VALIDATION_ERROR", f"response_format.type is {response_type}; use one of: {allowed}.")
    if response_type in {"text", "json_object"} and "json_schema" in value:
        raise ServiceError("VALIDATION_ERROR", f"response_format.json_schema is only allowed when type=json_schema, not {response_type}.")
    if response_type == "json_object":
        ...
        if isinstance(object_policy, dict) and object_policy.get("require_json_instruction") is True and not _messages_contain_json_instruction(payload):
            raise ServiceError("VALIDATION_ERROR", "response_format.type=json_object requires an explicit JSON instruction in messages; add a user or system message such as 'Return JSON.'.")
    if response_type != "json_schema":
        return
    schema_policy = _schema_policy(policy)
    if schema_policy.get("enabled", True) is not True:
        raise ServiceError("VALIDATION_ERROR", "response_format.type=json_schema is not enabled for this model; use response_format.type=text/json_object or switch profiles.")
    json_schema = value.get("json_schema")
    if not isinstance(json_schema, dict):
        raise ServiceError("VALIDATION_ERROR", "response_format.json_schema must be an object when type=json_schema.")
    ...
    if not isinstance(name, str) or not JSON_SCHEMA_NAME_RE.fullmatch(name):
        raise ServiceError("VALIDATION_ERROR", "response_format.json_schema.name must match ^[A-Za-z0-9_-]{1,64}$; use letters, numbers, '_' or '-' only.")
    if "description" in json_schema and not isinstance(json_schema["description"], str):
        raise ServiceError("VALIDATION_ERROR", "response_format.json_schema.description must be a string when provided.")
    ...
    if "strict" in json_schema:
        if not isinstance(json_schema["strict"], bool):
            raise ServiceError("VALIDATION_ERROR", "response_format.json_schema.strict must be boolean when provided.")
        if strict_policy.get("allowed", True) is not True:
            raise ServiceError("VALIDATION_ERROR", "response_format.json_schema.strict is not enabled for this model; remove strict or switch profiles.")
    if strict_policy.get("require_true") is True and json_schema.get("strict") is not True:
        raise ServiceError("VALIDATION_ERROR", "response_format.json_schema.strict must be true for this model; set strict=true.")
    schema = json_schema.get("schema")
    if not isinstance(schema, dict):
        raise ServiceError("VALIDATION_ERROR", "response_format.json_schema.schema must be an object.")
