...
- main-llm-vllm이 admin-sidecar 제어 API를 거치지 않고 재시작되는 경우(예: 운영자의 수동 `docker restart`)를 admin-sidecar가 영영 감지하지 못해 구조화 출력/미디어 웜업이 계속 빠지던 근본 gap을 수정했다. admin-sidecar에 30초 간격 reconciliation 루프(`reconcile_if_restarted()`)를 추가해, 컨테이너의 Docker `State.StartedAt`을 마지막으로 `validate()`했던 값과 비교하고 drift가 감지되면 gate를 닫지 않은 채로 `validate()`를 다시 돈다(재검증 자체는 수 초 내로 끝나므로 poll 간격만큼 트래픽을 막을 필요가 없다고 판단). 진행 중인 전환·복구 작업과는 기존 락으로 자동 직렬화된다. ([ADR-0018](docs/adr/0018-gpu-vram-admission-and-per-profile-runtime-image.md))
