...
- `vllm-gemma4-audio` 런타임 이미지를 `build-vllm-derived` CI 잡에서 risk-vllm-kanana와 함께 빌드·push하고(공용 vLLM base를 한 번만 pull), immutable digest를 `build/audio-image.env` 아티팩트로 산출한다. 오디오 활성화(12B 프로필 image 핀 + caps flip + switch canary)의 선행 작업이다. ([ADR-0018](docs/adr/0018-gpu-vram-admission-and-per-profile-runtime-image.md))
