...
| 경로 | 줄 수 | 담당 | 검토 메모 |
|---|---:|---|---|
| `reports/refactor/project_inventory_current.json` | 3405 | handoff-reporting | handoff/review artifact; avoid stale phase snapshots in active package |
| `tests/unit/test_gateway_app.py` | 932 | quality | test coverage; keep deterministic and avoid live GPU dependency |
...
| `reports/runtime/operator_status_bundle.json` | 623 | handoff-reporting | generated operator report; regenerate through make operator-reports before handoff |
| `ops/grafana/dashboards/executive_runtime_overview.json` | 606 | operations | supporting project file |
| `src/ai_model_serving/governance_validation/docs_ops.py` | 594 | governance-validation | application code; preserve public API behavior and compatibility facades |
...
| `scripts/models/modelctl.py` | 552 | operator-ux | operator/developer command; expose through Makefile/help if user-facing |
| `src/ai_model_serving/apps/gateway.py` | 479 | application | application code; preserve public API behavior and compatibility facades |
| `docs/resources/gpu_resource_requirements_48gb.md` | 426 | documentation | documentation; keep relative links valid and avoid duplicating current command flow |
| `specs/openapi.gateway.yaml` | 425 | api-contracts | supporting project file |
| `README.md` | 423 | project-maintenance | primary navigation or current-state entrypoint; keep synchronized with Makefile and reports |
