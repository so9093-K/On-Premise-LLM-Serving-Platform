...
# ready_local은 application base URL과 무관하게 항상 localhost를 확인한다.
# compose .env 파일의 service URL은 compose 내부 hostname일 수 있다.
GATEWAY_BASE_URL="http://localhost:${GATEWAY_PORT:-$(service_default_host_port gateway)}"
RISK_SIGNAL_SERVICE_BASE_URL="http://localhost:${RISK_SIGNAL_SERVICE_PORT:-$(service_default_host_port risk_adapter)}"
...
check_health risk_adapter "$RISK_SIGNAL_SERVICE_BASE_URL"
