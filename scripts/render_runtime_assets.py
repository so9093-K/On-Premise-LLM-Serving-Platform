...
def render_error_reference_md() -> str:
    ...
    lines = [
        ...
        "| param | 영역 | 예 |",
        "|---|---|---|",
        "| `response_format`, `response_format.json_schema` | 응답 형식 | JSON mode/schema 요청이 잘못됨. |",
        "| `messages`, `messages[0].role`, `messages[0].content`, `messages.content.type` | 메시지 | role/content/content part 구조가 잘못됨. |",
        ...
        "| `tools`, `tools[0].function.name`, `tool_choice`, `tool_choice.function.name`, `tool_calls` | Tool | tool schema나 선택한 tool 이름이 맞지 않음. |",
        "| `image_url`, `image_url.url`, `image_url.detail` | 이미지 | URL/MIME/base64/크기/픽셀/animated image 정책 위반. |",
        "| `input_audio`, `input_audio.format` | 오디오 | format/base64/크기/magic-byte/capability 문제. |",
        "| `video_url`, `video_url.url` | 비디오 | URL/MIME/base64/크기/frame/capability 문제. |",
        ...
        '  "error": {',
        '    "code": "VALIDATION_ERROR",',
        '    "message": "tool_choice.function.name must match one of the provided tools: get_weather.",',
        ...
        '    "message": "structured output did not match response_format.json_schema.",',
        '    "retryable": true,',
        '    "request_id": "req_..."',
        "  }",
        "}",
        ...
        "```text",
        "event: error",
        'data: {"error":{"code":"STREAM_LIMIT_EXCEEDED","message":"stream byte limit exceeded.","retryable":false,"request_id":"req_..."}}',
        "```",
        "",
        "## Chat에서 주로 보는 code",
        "",
