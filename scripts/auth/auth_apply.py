...
def main(argv: list[str] | None = None) -> int:
    ...
    print(f"안내: 실행 중인 컨테이너에 적용하려면 gateway와 risk-signal-service를 재시작하세요.")
    print(f"  docker compose -f ops/compose/full-stack.private-network.yaml --env-file .env up -d --no-deps gateway risk-signal-service")
    return 0
