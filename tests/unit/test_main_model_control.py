...
def test_gpu_util_override_from_mapping_parses_and_validates():
    ...
    # Legacy read compatibility remains until the namespace migration window closes.
    assert f({"MAIN_LLM_GPU_MEMORY_UTILIZATION": "0.8"}) == 0.8
    with pytest.raises(MainModelConfigurationError, match="conflicting env keys"):
        f(
            {
                "MAIN_MODEL_GPU_MEMORY_UTILIZATION": "0.9",
                "MAIN_LLM_GPU_MEMORY_UTILIZATION": "0.8",
            }
        )
