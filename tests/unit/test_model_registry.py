...
def test_model_registry_projects_runtime_services_and_contracts():
    ...
    assert list(services) == ["main_llm", "embedding", "risk_prompt", "risk_siren"]
...
def test_model_registry_projects_model_cards_and_runtime_validation_matrix():
    ...
    assert card.upstream_model_id == "QuantTrio/gemma-4-31B-it-AWQ"
    assert card.runtime["port"] == 9401
    assert card.source_facts["base_model_id"] == "google/gemma-4-31B-it"
    ...
    assert targets["risk_siren"]["compose_service_name"] == "risk-siren-vllm"

    matrix = {item["id"]: item for item in registry.runtime_validation_matrix_document()["validation_checks"]}
    assert matrix["gateway-runtime"]["models"] == ["local-main", "local-embed", "risk-prompt", "risk-siren"]
    assert matrix["vllm-runtime"]["runtime_services"] == ["main_llm", "embedding", "risk_prompt", "risk_siren"]
    assert matrix["grafana-dashboard-render"]["runtime_services"] == ["main_llm", "embedding", "risk_prompt", "risk_siren"]
...
def test_model_registry_projects_openapi_schema_and_monitoring_labels():
    ...
    assert item_properties["id"]["enum"] == ["local-main", "local-embed", "risk-prompt", "risk-siren"]
    ...
    assert registry.monitoring_compose_service_regex() == "main-llm-vllm|embedding-vllm|risk-prompt-vllm|risk-siren-vllm"
