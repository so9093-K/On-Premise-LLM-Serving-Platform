...
from scripts.build import check_dev_environment, setup_python_environment
...
def test_setup_preserves_unknown_existing_environment(tmp_path):
    ...
    with pytest.raises(RuntimeError, match="inspect it"):
        setup_python_environment._check_existing_environment(tmp_path, tmp_path / "python")
...
def test_setup_refuses_different_existing_python_minor(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(setup_python_environment, "_interpreter_minor", lambda _: next(versions))

    with pytest.raises(RuntimeError, match="Existing .venv uses Python 3.12"):
        setup_python_environment._check_existing_environment(tmp_path, tmp_path / "python3.13")
...
def test_setup_checks_python_before_environment_changes(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(setup_python_environment.subprocess, "run", fail_python_check)
    assert setup_python_environment.main(["--profile", "development"]) == 2
